...
# Función para mejorar la presentación de la respuesta
def rebajar(text):
    text = text.replace('•', '  *')
    return Markdown(textwrap.indent(text, '> ', predicate=lambda _: True))

# Título de la aplicación Streamlit
st.title("Asistente Virtual VETMEDicito")

# Instrucciones para el usuario
st.write("Bienvenido a VetMed. Este es VETMEDicito, tu asistente virtual para información comercial sobre la veterinaria.")

# Entrada de texto para el usuario
user_prompt = st.text_input("Haz tu pregunta sobre VetMed:")

# Si el usuario ingresa un texto, generamos contenido con el modelo Gemini Pro
if user_prompt:
...
Basándote en esa información, responde la siguiente pregunta sobre VetMed: {user_prompt}
"""
    ...
    if "Lo siento" in respuesta or "no encontré información" in respuesta or "no encontré información" in respuesta or "texto proporcionado" in respuesta or "Esa información" in respuesta or "información" in respuesta:
        st.markdown(rebajar(f"""**Lo siento, no encontré información suficiente en mi base de datos para responder tu pregunta con precisión.** 

Para brindarte la mejor atención, te recomiendo que te acerques a una de nuestras sucursales o nos llames al 0997545626. 

Nuestros veterinarios expertos estarán encantados de atenderte y resolver todas tus dudas.**""").data)
    else:
        st.markdown(rebajar(respuesta).data)
